...
class Encrypter
{
    ...
    {
        $key = getenv('CYRON_ENCRYPTION_KEY') ?: '';
        if (strlen($key) < 32) throw new \RuntimeException('CYRON_ENCRYPTION_KEY must contain at least 32 characters.');
    ...
    public static function decrypt(string $payload): string
    {
        ...
        if ($data === false || strlen($data) < 29) throw new \InvalidArgumentException('Invalid encrypted payload.');
        $iv = substr($data, 0, 12);
