...
<div x-show="showSubscriptionModal"
    ...
    @click.away="showSubscriptionModal = false"
    style="display: none;">

    <div class="w-11/12 max-w-md rounded-2xl bg-white dark:bg-zinc-800 p-6 shadow-2xl"
        @click.stop>

        <div class="flex justify-between items-center mb-4">
            <h3 class="text-2xl font-bold text-gray-800 dark:text-white"><i class="fa fa-gift text-lime-500"></i> انتخاب اشتراک</h3>
        ...
        <!-- گزینه‌های اشتراک -->
        <div class="space-y-3">
            <label class="flex cursor-pointer items-center justify-between rounded-lg border-2 border-transparent bg-lime-50 p-3 dark:bg-zinc-700 has-[:checked]:border-lime-500">
                <div>
                    <p class="font-bold">اشتراک ماهانه</p>
                    <p class="text-sm text-gray-500">۲۹,۰۰۰ تومان / ماه</p>
                </div>
                <input type="radio" name="plan" value="monthly" class="h-5 w-5 text-lime-500">
            </label>

            <label class="flex cursor-pointer items-center justify-between rounded-lg border-2 border-transparent bg-lime-50 p-3 dark:bg-zinc-700 has-[:checked]:border-lime-500">
                <div>
                    <p class="font-bold">اشتراک سالانه <span class="rounded-full bg-green-100 px-2 py-0.5 text-xs text-green-700">-۲۰٪ تخفیف</span></p>
                    <p class="text-sm text-gray-500">۲۷۹,۰۰۰ تومان / سال</p>
                </div>
                <input type="radio" name="plan" value="yearly" class="h-5 w-5 text-lime-500">
            </label>
        </div>

        <button class="mt-6 w-full rounded-lg bg-gradient-to-r from-lime-500 to-lime-700 py-3 font-bold text-white transition hover:opacity-90">
            ادامه و پرداخت
        </button>

        <p class="mt-4 text-center text-xs text-gray-400">
            <a href="/subscriptions" class="text-lime-600 hover:underline">مشاهده همه اشتراک‌ها</a>
        </p>
    </div>
</div>
...
<!-- <section class="hero-section dark:text-white bg-gradient-to-br from-slate-700 to-gray-900"> -->
<section class="relative mx-auto py-14 mt-5 mb-8 dark:text-white">
    <div class="hero-bg-pattern"></div>
    <div class="container mx-auto px-4">
        <div class="flex flex-col lg:flex-row items-center justify-between">
            <div class="hero-content lg:w-5/6 mb-10 lg:mb-0">
                <span class="inline-block bg-lime-100 dark:bg-lime-900/40 text-lime-700 dark:text-lime-300 rounded-full px-4 py-1 text-sm mb-4">✨ بیش از ۱۰۰۰۰ کتاب الکترونیک</span>
                <h1 class="text-3xl md:text-6xl font-titr mb-4 leading-tight">
                    دنیای کتاب‌ها را در <span class="text-lime-500 dark:text-lime-300">کلبه کتاب</span> کشف کنید
                </h1>
                ...
                <div class="flex flex-col sm:flex-row gap-4">
                    <a href="{{ route('register') }}"
                        class="cta-button bg-gradient-to-r from-lime-200 to-lime-400 text-black text-center">
                        همین حالا شروع کنید
                    ...
                    <div class="carousel-book book-center">
                        ...
                        <div class="carousel-book book-right">
                            ...
                            <a href="{{ route('book', ['slug' => $low_book->slug]) }}">
                                <img src="{{ storage_url('/book/' . $low_book->cover) }}" loading="lazy"
                                    alt="{{ $low_book->title }}">
                            </a>
                        </div>
                        @endforeach
...
<x-book-slider title="جدیدترین‌ها" icon="fa fa-fire" :targetId="'scroll-container-1'" :seeAllUrl="route('category.books', ['category' => 'newest'])">
    ...
    <div class="w-40 md:w-48">
        <a class="" href="{{ route('book', ['slug' => $book->slug]) }}" title="{{ $book->title }}"
            ...
            <img alt="{{ $book->title }}" class="rounded-lg mb-2 w-full h-56 md:h-64" height="200"
                src="{{ storage_url('/book/' . $book->cover) }}" loading="lazy" width="150" />
            <div class="text-center mb-2">
                ...
                @else
                <p class="text-md text-gray-600 dark:text-gray-500 m-0"><span
...
<x-book-slider title="کتاب‌های رایگان" icon="fa fa-gift" :targetId="'scroll-container-2'" :seeAllUrl="route('category.books', ['category' => 'free'])">
