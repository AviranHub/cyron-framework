...
class Collection implements ArrayAccess, Countable, IteratorAggregate
{
    ...
    public function take(int $limit): self { return new static(array_slice($this->items, 0, $limit)); }
    public function count(): int { return count($this->items); }
    public function pluck(string $key): array { return array_map(function ($item) use ($key) { return is_array($item) ? ($item[$key] ?? null) : ($item->{$key} ?? null); }, $this->items); }
    public function map(callable $callback): self { return new static(array_map($callback, $this->items)); }
