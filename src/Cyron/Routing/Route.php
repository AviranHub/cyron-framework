...
class Route
{
    ...
    public static function run()
    {
        ...
        // Debug logging
        $debugUri = $uri;
        error_log("[ROUTE_RUN] Method: $method, URI: $debugUri, Routes: " . count($instance->routes));
        ...
        foreach ($instance->routes as $i => $route) {
            ...
            if ($matchResult === 1) {
                error_log("[ROUTE_RUN] Route #$i matched: " . $route['uri']);
                $params = array_filter($matches, 'is_string', ARRAY_FILTER_USE_KEY);
                error_log("[ROUTE_RUN] Params: " . json_encode($params));

                $middlewares = array_merge(self::$globalMiddlewares, $route['middlewares']);
                $request = new \Cyron\Http\Request();
                ...
                error_log("[ROUTE_RUN] Response object: " . (is_object($response) ? get_class($response) : gettype($response)));
                
                if ($response !== null) {
                    if (is_object($response) && method_exists($response, 'send')) {
                        error_log("[ROUTE_RUN] Calling send() on response");
                        $response->send();
                    } else {
                        error_log("[ROUTE_RUN] Echo-ing response: " . substr((string)$response, 0, 100));
                        echo $response;
                    }
                }
                return;
        ...
        error_log("[ROUTE_RUN] No route matched for $method $uri");
    ...
    //                 if (class_exists($className) && is_subclass_of($className, \Cyron\Database\Model::class)) {
    ...
    {
        error_log("[executeAction] Called with action: " . json_encode($action) . ", params: " . json_encode($routeParams));
        ...
        if (is_array($action) && count($action) >= 2) {
            ...
            $method = $action[1];
            $constructorArgs = $action[2] ?? [];

            error_log("[executeAction] Creating controller: $controllerClass, method: $method");
            ...
            try {
                ...
                foreach ($reflectionMethod->getParameters() as $param) {
                    ...
                    error_log("[executeAction] Param: $paramName, Type: " . ($paramType ? $paramType->getName() : 'null'));

                    if ($paramType && $paramType->getName() === 'Cyron\Http\Request') {
                        $args[] = $request ?? new \Cyron\Http\Request();
                        continue;
                    }
                    ...
                    $value = array_shift($routeParamsCopy);
                    error_log("[executeAction] Adding arg for $paramName: " . json_encode($value));
                    $args[] = $value;
                }

                error_log("[executeAction] Calling method with " . count($args) . " args");
                return $reflectionMethod->invokeArgs($controller, $args);
            } catch (\Throwable $e) {
                error_log("[executeAction] Exception: " . $e::class . " - " . $e->getMessage());
                throw $e;
            }
        }
        if (is_callable($action)) {
    ...
    protected static function resolveMiddlewareClass(string $shortName): string
    {
        $candidates = [
            '\\App\\Http\\Middlewares\\' . $shortName,
            '\\App\\Http\\Middleware\\' . $shortName,
            '\\Cyron\\Http\\Middlewares\\' . $shortName,
        ];
        ...
        throw new \RuntimeException('Middleware class not found: ' . $shortName);
    }

    public static function auth()
    {
        return self::middleware(self::resolveMiddlewareClass('AuthMiddleware'));
    }

    public static function permission(string $ability)
    {
        return self::middleware(self::resolveMiddlewareClass('PermissionMiddleware') . ':' . $ability);
    }
