...
use Cyron\Authentication\TokenGenerator;
...
class Tokenizer
{
    ...
    {
        return TokenGenerator::make($bytes);
    }
     ...
     */
    public static function getUserByAccessToken(string $token): ?object
    {
        $record = self::tokenModel()::where('token', '=', self::hash($token))->first();
    ...
    {
        $record = self::tokenModel()::where('refresh_token', '=', self::hash($refreshToken))->first();
    ...
    {
        $records = self::tokenModel()::where('user_id', '=', $userId)->get();
        $count = 0;
    ...
    {
        $records = self::tokenModel()::where('user_id', '=', $userId)
    ...
    {
        $record = self::tokenModel()::where('token', '=', self::hash($token))
    ...
    private static function tokenModel(): string
    {
        return ModelRegistry::get('personal_access_token');
    }
}