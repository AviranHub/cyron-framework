...
class Console
{
    ...
    protected function registerCommands()
    {
        ...
        foreach ($this->commands as $name => $className) {
            $this->commands[$name] = __NAMESPACE__ . '\\Commands\\' . $className;
        }

        foreach (CommandLoader::commands() as $name => $className) {
            $this->registerCommand($name, $className);
        }
    }

    public function registerCommand($name, $className)
    {
    ...
    protected function showHelp()
    {
        ...
        foreach ($groups as $category => $commands) {
            ...
            foreach ($commands as $command => $class) {
                ...
                echo Colors::brightCyan($prefix) . "    " . Colors::brightGreen(str_pad($command, 24)) . " " . $desc . "\n";
            }
        }
        echo "\n";
        $this->showExamples();
    ...
    protected function groupCommands()
    {
        ...
        foreach ($this->commands as $command => $class) {
            ...
            } elseif (str_starts_with($command, 'route:') || str_starts_with($command, 'dev:')) {
