...
class PluginListCommand
{
    ...
    {
        \Cyron\Plugin\PluginManager::setPath(APP_PATH . '/Plugins');
        \Cyron\Plugin\PluginManager::discover();

        $allPlugins = \Cyron\Plugin\PluginManager::all();
        if (empty($allPlugins)) {
        ...
        foreach ($allPlugins as $name => $info) {
            $statusText = \Cyron\Plugin\PluginManager::isActive($name) ? '● Active' : '○ Inactive';
            $statusDisplay = \Cyron\Plugin\PluginManager::isActive($name) 
