 ...
 public static function record(string $action,array $context=[],?int $actorId=null):void{
  ...
  $context=array_merge([
   'request'=>[
    ...
    $model=ModelRegistry::get('audit_log');
    $model::create(['actor_id'=>$actorId,'action'=>$action,'context'=>json_encode($context,JSON_UNESCAPED_UNICODE),'occurred_at'=>date('Y-m-d H:i:s')]);
 }
}